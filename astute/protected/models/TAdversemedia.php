...
/**
 * This is the model class for table "t_adversemedia".
 *
 * The followings are the available columns in table 't_adversemedia':
 * @property integer $id
 ...
 */
class TAdversemedia extends CActiveRecord
{
	/**
	...
	public function rules()
	{
		...
		return array(
			array('page, authors, title, url, publisher, access_date, maker', 'required'),
			array('page', 'numerical', 'integerOnly'=>true),
			array('authors, title, publisher, ref_publisher', 'length', 'max'=>255),
			...
			array('id, page, authors, publish_date, title, url, publisher, ref_publisher, access_date, createdon, maker, data_entrant, status, rejected_reason_dataEntry, discard_reason_researcher', 'safe', 'on'=>'search'),
		);
	}
	 ...
	 * @return TAdversemedia the static model class
	 */
